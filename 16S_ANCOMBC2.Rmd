...
############
October 2025 CMM added additional ANCOMBC2 analyses to address R1 comments. Here we performed within gut section analysis by diet type and percent protein. We also tested for maternal effects.  


#ANCOMBC2 by Location- E1 vs E2 for all locations samples by ASV
```{r}
output = ancombc2(
  ps_raw_E1_E2_forcomparison,
  assay_name = NULL,
  tax_level = NULL, #keep null if you want at ASV level
  fix_formula= "Percent_Protein + Exp_Location",
  rand_formula = NULL,
  p_adj_method = "holm",
  prv_cut = 0.1,
  lib_cut = 0,
  s0_perc = 0.05,
  group = "Exp_Location", #leave as null if group variable of interest contains only two categories 
  struc_zero = FALSE,
  neg_lb = FALSE,
  alpha = 0.05,
  n_cl = 1,
...
```{r}
res_pair = output$res_pair
saveRDS(res_pair, "/Users/connermertz/Dropbox (vlab)/Conner Mertz/UNM biology/Chapter 1 E1 vs E2/Methods/16S rRNA gene sequencing/Sequencing_workup/ANCOMoutput/codeexports/E1_E2_forcomparison/redo/E1vsE2_LOCATION_res_pair.rds")
E1vsE2_LOCATION_res_pair <- readRDS("/Users/connermertz/Dropbox (vlab)/Conner Mertz/UNM biology/Chapter 1 E1 vs E2/Methods/16S rRNA gene sequencing/Sequencing_workup/ANCOMoutput/codeexports/E1_E2_forcomparison/redo/E1vsE2_LOCATION_res_pair.rds")
...
Save the df you created
```{r}
write.csv(df_fig_trend, "/Users/connermertz/Dropbox (vlab)/Conner Mertz/UNM biology/Chapter 1 E1 vs E2/Methods/16S rRNA gene sequencing/Sequencing_workup/ANCOMoutput/codeexports/E1_E2_forcomparison/redo/E1vsE2_LOCATION_df_fig_trend_redo.csv") #ADD IN OTHER CATEGORIES YOU WANT TO GRAPH 

#read in your EDITED DF csv: 
E1vsE2_LOCATION_df_fig_trend <- read.csv("/Users/connermertz/Dropbox (vlab)/Conner Mertz/UNM biology/Chapter 1 E1 vs E2/Methods/16S rRNA gene sequencing/Sequencing_workup/ANCOMoutput/codeexports/E1_E2_forcomparison/redo/E1vsE2_LOCATION_df_fig_trend_redo.csv")
#change name so target_ids match on both files
names(E1vsE2_LOCATION_df_fig_trend)[2] ="taxon" 
...
#write.csv(Part_DF,"/Users/connermertz/Dropbox (vlab)/Conner Mertz/UNM biology/Chapter 1 E1 vs E2/Methods/16S rRNA gene sequencing/Sequencing_workup/ANCOMoutput/codeexports/E1_E2_forcomparison/redo/E1vsE2_LOCATION_DF_FIG_TREND_AND_Taxonomy_redo.csv") # went in here and edited file to my liking. I saved the new version as edit_E1vsE2_LOCATION_DF_FIG_TREND_AND_Taxonomy.csv. That is what I will be using for the figures. I basically switched the signs for the differential abundant taxa to be positive for E1 and negative for E2 but consistent with whatever category it was being compared to. 
...
#read in the edited df
```{r}
E1vsE2_df <- read.csv("/Users/connermertz/Dropbox (vlab)/Conner Mertz/UNM biology/Chapter 1 E1 vs E2/Methods/16S rRNA gene sequencing/Sequencing_workup/ANCOMoutput/codeexports/E1_E2_forcomparison/redo/edit_E1vsE2_LOCATION_DF_FIG_TREND_AND_Taxonomy_redo.csv")
```
...
```{r}
E1vsE2_LOCATION_df1 <- read.csv("/Users/connermertz/vlab Dropbox/Vesbach Lab/Conner Mertz/UNM biology/Chapter 1 E1 vs E2/Methods/16S rRNA gene sequencing/Sequencing_workup/ANCOMoutput/codeexports/E1_E2_forcomparison/redo/importLocationFig.csv") %>%
  filter(  group == "E2_DUOD - E1_DUOD" |
           group == "E2_CEC - E1_CEC" |
           group == "E2_IL - E1_IL" |
           group == "E1_DUOD - E2_DUOD" |
           group == "E1_CEC - E2_CEC" |
           group == "E1_IL - E2_IL" )%>%
 mutate(Differentially_Abundant_Group = factor(Differentially_Abundant_Group, levels = c("E1_DUOD","E1_IL", "E1_CEC", "E2_DUOD","E2_IL", "E2_CEC"))) %>%
 mutate(Differentially_Abundant_Location = factor(Differentially_Abundant_Location, levels = c("DUOD","IL", "CEC"))) %>%
...
#When we compared the synthetic vs the seminatural in each gut section we found:
After taking out mice <95days in study:
E1_duod vs E2_duod: in the duodenum ASV6 and ASV114 was found to be differentially abundant in the synthetic diet compared to the semi-natural diet.  
E1_ile vs E2_ile: in the ileum ASV5 and ASV17 were found to be differentially abundant in the semi-natural diet compared to the synthetic diet. 
E1_cec vs E2_cec: In the cecum ASV5, ASV17, and ASV9 are differentially abundant in the semiinatural diet compared to the synthetic diet. 

```{r}
#first you want to factor your items. These are the names of the Location in the order I want them to be every time. 
E1vsE2_LOCATION_df1$LocationvsLocation <-factor(E1vsE2_LOCATION_df1$LocationvsLocation, levels=c("DUODvsDUOD","ILvsIL", "CECvsCEC"))
#plot it
fig_trend = E1vsE2_LOCATION_df1 %>% 
  ggplot(aes(x = lfc_figure, y = taxon, color = Differentially_Abundant_Location, shape = Differentially_Abundant_Experiment)) + 
  geom_point(size = 4, position = position_dodge()) +
  geom_errorbar(
    aes(xmin = lfc_figure - se, xmax = lfc_figure + se), 
    width = .2) +
  geom_vline(aes(xintercept = 0), linetype = "dashed")+
  facet_wrap(~LocationvsLocation, strip.position="top") +
  labs(x = "Log Fold Change", y = "ASV", title = "Differentially Abundant Taxa") +
...
#CREATE A PDF AND SAVES IT TO THE RIGHT DIRECTORY/FOLDER. 
pdf("/Users/connermertz/vlab Dropbox/Vesbach Lab/Conner Mertz/UNM biology/Chapter 1 E1 vs E2/Methods/16S rRNA gene sequencing/Sequencing_workup/ANCOMoutput/ISMERevisions/plots/E1_E2_LocationComps_redo.pdf", height = 4, width = 8)
fig_trend
dev.off()
```

#####
now lets expand our data set to include more comparisons between E1 vs E2 in variious get sections
```{r}
E1vsE2_LOCATION_df <- read.csv("/Users/connermertz/vlab Dropbox/Vesbach Lab/Conner Mertz/UNM biology/Chapter 1 E1 vs E2/Methods/16S rRNA gene sequencing/Sequencing_workup/ANCOMoutput/codeexports/E1_E2_forcomparison/redo/importLocationFig.csv") %>%
  filter(group ==  "E1_DUOD - E1_IL" |
           group == "E1_IL - E1_DUOD" |
           group == "E1_DUOD - E1_CEC" |
           group == "E1_CEC - E1_DUOD" |
         ...
         group ==  "E2_DUOD - E2_IL" |
           ...
           group == "E2_CEC - E2_DUOD" |
           group == "E2_IL - E2_CEC" |
           group == "E2_CEC - E2_IL" | #edited from here down
           group == "E2_DUOD - E1_CEC" |
           group == "E2_IL - E1_CEC" |
           group == "E2_DUOD - E1_IL" |
           group == "E2_CEC - E1_IL" |
...
```{r}
#first you want to factor your items. These are the names of the Location in the order I want them to be every time. 
E1vsE2_LOCATION_df$LocationvsLocation <-factor(E1vsE2_LOCATION_df$LocationvsLocation, levels=c("DUODvsIL","CECvsIL", "DUODvsCEC"))
...
#CREATE A PDF AND SAVES IT TO THE RIGHT DIRECTORY/FOLDER. 
#pdf("/Users/connermertz/vlab Dropbox/Vesbach Lab/Conner Mertz/UNM biology/Chapter 1 E1 vs E2/Methods/16S rRNA gene sequencing/Sequencing_workup/ANCOMoutput/ISMERevisions/plots/E1_E2_Location_redo.pdf", height = 11, width = 8)
fig_trend
#dev.off()
```

#trying to separate out overlapping pointd
...
tolerance <- 6.0      # expand this to 0.15 or 0.2 if needed
jitter_strength <- 0.5   # how far to push E1 left / E2 right

E1vsE2_LOCATION_df <- E1vsE2_LOCATION_df %>%
  group_by(taxon) %>%
  mutate(
    # Count how many points have x-values VERY close
    n_overlaps = sapply(lfc_figure, function(x)
      sum(abs(lfc_figure - x) < tolerance)
    )
  ) %>%
  ungroup() %>%
  mutate(
    # jitter only when at least 2 points fall within tolerance
    jitter_offset = case_when(
      n_overlaps < 2 ~ 0,
      Differentially_Abundant_Experiment == "E1" ~ -jitter_strength,
      Differentially_Abundant_Experiment == "E2" ~  jitter_strength,
      TRUE ~ 0
    ),
    x_jittered = lfc_figure + jitter_offset
  )
```
...
fig_trend = E1vsE2_LOCATION_df %>% 
  ggplot(aes(x = x_jittered, 
             y = taxon, 
             color = Differentially_Abundant_Location,
             shape = Differentially_Abundant_Experiment)) +
  
  geom_point(size = 4) +
  
  geom_errorbar(
    aes(xmin = x_jittered - se, xmax = x_jittered + se),
    width = .2
  ) +
  ...
  labs(x = "Log Fold Change", y = "ASV", title = "Differentially Abundant Taxa") +
  LocationColors +
  scale_shape_manual(values = c("E1" = 16, "E2" = 17)) +
  scale_x_continuous(limits = c(-9, 9)) +
  theme_bw() +
  theme(
    plot.title = element_text(hjust = 0.5),
...
Now by percent protein:

E1 E2 Protein in each Gut Section

lets see if we can't join E1 and E2 Protein results together!
DUOD, 
#skip b/c we didn't get any differentially abundant ASVs in E1 duod or E2 DUOD
```{r}
...
Cec
```{r}
E1vsE2_CEC_Protein_df <- read.csv("/Users/connermertz/vlab Dropbox/Vesbach Lab/Conner Mertz/UNM biology/Chapter 1 E1 vs E2/Methods/16S rRNA gene sequencing/Sequencing_workup/ANCOMoutput/ISMERevisions/combined_DF_figs/redo/edit_E1_E2_CECUM_PROTEIN_DF_redo.csv") %>%
  filter(group ==  "E1_2.5P - E1_5P" |
           group == "E1_5P - E1_2.5P" |
           group == "E1_2.5P - E1_10P" |
           group == "E1_10P - E1_2.5P" |
...
```{r}
#first you want to factor your items.  
E1vsE2_CEC_Protein_df$PercentProteinvsPercentProtein <-factor(E1vsE2_CEC_Protein_df$PercentProteinvsPercentProtein, levels = c("5Pvs2.5P","2.5Pvs10P", "5Pvs10P"))
...
fig_trend = E1vsE2_CEC_Protein_df %>% 
  ...
  ProteinColors.3 +
  scale_shape_manual(
    values = c("Synthetic" = 16,  # circle
               "Seminatural" = 17)  # triangle
  ) +
  scale_x_continuous(limits=c(-9, 9)) +
  theme_bw() +
  theme(
...
#CREATE A PDF AND SAVES IT TO THE RIGHT DIRECTORY/FOLDER. 
pdf("/Users/connermertz/vlab Dropbox/Vesbach Lab/Conner Mertz/UNM biology/Chapter 1 E1 vs E2/Methods/16S rRNA gene sequencing/Sequencing_workup/ANCOMoutput/ISMERevisions/plots/E1vsE2_CEC_Protein_redo.pdf", height = 8, width = 8)
fig_trend
dev.off()
```
...
ILE
```{r}
E1vsE2_ILE_Protein_df <- read.csv("/Users/connermertz/vlab Dropbox/Vesbach Lab/Conner Mertz/UNM biology/Chapter 1 E1 vs E2/Methods/16S rRNA gene sequencing/Sequencing_workup/ANCOMoutput/ISMERevisions/combined_DF_figs/redo/edit_E1_E2_ILE_PROTEIN_DF_redo.csv") %>%
  filter(group ==  "E1_2.5P - E1_5P" |
           group == "E1_5P - E1_2.5P" |
           group == "E1_2.5P - E1_10P" |
           group == "E1_10P - E1_2.5P" |
  ...
  mutate(Differentially_Abundant_Group = factor(Differentially_Abundant_Group, levels = c("E1_2.5P","E1_5P", "E1_10P", "E2_2.5P","E2_5P", "E2_10P"))) %>%
  mutate(Differentially_Abundant_Protein = factor(Differentially_Abundant_Protein, levels = c("2.5P","5P", "10P"))) %>%
  mutate(taxon = factor(taxon, levels = c("ASV80",
"ASV7",
"ASV25",
"ASV66",
"ASV96",
...
```{r}
#first you want to factor your items.  
E1vsE2_ILE_Protein_df$PercentProteinvsPercentProtein <-factor(E1vsE2_ILE_Protein_df$PercentProteinvsPercentProtein, levels = c("5Pvs2.5P","2.5Pvs10P", "5Pvs10P"))
#plot
...
#CREATE A PDF AND SAVES IT TO THE RIGHT DIRECTORY/FOLDER. 
pdf("/Users/connermertz/vlab Dropbox/Vesbach Lab/Conner Mertz/UNM biology/Chapter 1 E1 vs E2/Methods/16S rRNA gene sequencing/Sequencing_workup/ANCOMoutput/ISMERevisions/plots/E1vsE2_ILE_Protein_redo.pdf", height = 9, width = 8)
fig_trend
dev.off()
```
...
output = ancombc2(
  ...
  assay_name = NULL,
  tax_level = NULL, #keep null if you want at ASV level
  fix_formula= "Breeding.pair",
  rand_formula = NULL,
  p_adj_method = "holm",
  prv_cut = 0.1,
  lib_cut = 0,
  s0_perc = 0.05,
  group = "Breeding.pair", #leave as null if group variable of interest contains only two categories 
  struc_zero = FALSE,
  neg_lb = FALSE,
  alpha = 0.05,
  n_cl = 1,
...
```
#no differentially abundant ASVs present in the entire data set, or within diet type, or within each gut section. 

...
---
Script for alpha and beta diversity analyses for microbiome and diet data. Includes code for 16S alpha and beta diversity figures, and overall relative abundance figure. Code for Random Forest Analysis is also included. 
...
ps_raw <- subset_samples(ps_raw, Days_in_study > 95) #keep only mice in study past 95 days
...
ps_all <- subset_samples(ps_all, Days_in_study > 95) #keep only mice in study past 95 days
sample_data(ps_raw) <- Meta #If you need to update your meta sheet, this is how to do it without dunning dada2 again to create a whole new ps
sample_data(ps_all) <- Meta #If you need to update your meta sheet, this is how to do it without dunning dada2 again to create a whole new ps
```
...
R1 suggests we make these results clearer by emphasizing relative abundance within each gut section. So instead of by mouse_number, I am going to collapse by percent protein.
...
E1_E2_relAb <- ggplot(E1_E2_phyla_ab1, aes(x = Exp_Diet_Location, y = Abundance*100, fill = Phylum)) +
  guides(fill = guide_legend(title.position = "top")) +
  labs(x = "Animal ID", y = "Relative Abundance (Phyla > 1%) ", fill = "Phylum") +
  labs(title = "Bacterial Phyla")+
  facet_wrap(Experiment~Location+Percent_Protein, scale = "free_x",  nrow= 2 , ncol= 9) +
...
#CREATE A PDF AND SAVES IT TO THE RIGHT DIRECTORY/FOLDER. 
pdf("/Users/connermertz/Dropbox (vlab)/Conner Mertz/UNM biology/Chapter 1 E1 vs E2/Methods/16S rRNA gene sequencing/Sequencing_workup/BoxplotsANDScatter_RelAbundance/E1_E2_PhylaRelPhylaAbove1_collapse.pdf", height = 15, width = 24)
E1_E2_relAb
dev.off()
```
...
Random Forest
####Random Forests
start with this one! https://rpubs.com/michberr/randomforestmicrobe #but we need to switch our response= ASV/OTU table  and predictor=Factors of interest(experiment, Diet, Protein, Maternal effects) variables!

other tutorials worth a shot if needed:
...
https://www.researchgate.net/post/How-to-use-Random-Forest-method-to-analyze-gut-microbiota
...
read in data used in random forest analysis
```{r}
ps_raw <- readRDS("/Users/connermertz/Dropbox (vlab)/Conner Mertz/UNM biology/Chapter 1 E1 vs E2/Methods/16S rRNA gene sequencing/Sequencing_workup/DADA2_output/clean_ps.rds") #raw but with choloro,mito, low reads (less than 2000), and sample duplicates removed
ps_raw
ps_raw <- subset_samples(ps_raw, Days_in_study > 95)
...
#now you are ready to run the RF algorithm.
```{r}
#Random Forest code
...
# Prune rare taxa
prunescale <- 0.0001
minlib <- min(sample_sums(ps_raw_E1_E2_forcomparison))
sites.mean <- taxa_sums(ps_raw_E1_E2_forcomparison) / nsamples(ps_raw_E1_E2_forcomparison)
sites.prune <- prune_taxa(sites.mean > prunescale * minlib, ps_raw_E1_E2_forcomparison)


# --- Prepare data ---
otu_df <- as.data.frame((otu_table(sites.prune))) 
meta_df <- as.data.frame(sample_data(sites.prune))

factors <- c("Experiment", "Percent_Protein", "Location", "Breeding.Pair", "Sex")

set.seed(123)  # reproducibility

results <- lapply(factors, function(fac) {
  y <- meta_df[[fac]]
  if (is.numeric(y)) {
    y <- as.numeric(y)
  } else {
  ...
  # RF Model on actual data
  rf_real <- randomForest(x = otu_df, y = y, ntree = 1000, importance = TRUE)
  imp <- importance(rf_real)

  OOB_real <- rf_real$err.rate[rf_real$ntree, "OOB"]
  MDA_total <- mean(imp[, "MeanDecreaseAccuracy"], na.rm = TRUE)
  MDA_norm <- MDA_total / ncol(otu_df)
  Gini_total <- mean(imp[, "MeanDecreaseGini"], na.rm = TRUE)

  #Generate 100 permutations for random error
  perm_OOB <- replicate(100, {
    y_perm <- sample(y)
    rf_perm <- randomForest(x = otu_df, y = y_perm, ntree = 1000)
    rf_perm$err.rate[rf_perm$ntree, "OOB"]
  })

  Random_Error_mean <- mean(perm_OOB)
  Random_Error_sd <- sd(perm_OOB)

  #Calc errors to compare between factors
  Error_Ratio <- Random_Error_mean / OOB_real
  Predictive_Accuracy <- 1 - (OOB_real / Random_Error_mean)

#make data frame of all results
  data.frame(
    Factor = fac,
    OOB_Error = OOB_real,
    Random_Error_mean = Random_Error_mean,
    Random_Error_sd = Random_Error_sd,
    Error_Ratio = Error_Ratio,
    Predictive_Accuracy = Predictive_Accuracy,
    MeanDecreaseAccuracy_Total = MDA_total,
    MeanDecreaseAccuracy_Normalized = MDA_norm,
    MeanDecreaseGini_Total = Gini_total,
    stringsAsFactors = FALSE
  )
})

results_df <- do.call(rbind, results) %>%
  arrange(desc(Predictive_Accuracy))

print(results_df)
#save and look at results
print(results_df)
write.csv(results_df, "/Users/connermertz/vlab Dropbox/Vesbach Lab/Conner Mertz/UNM biology/Chapter 1 E1 vs E2/Methods/16S rRNA gene sequencing/Sequencing_workup/RandomForestOutput/MaternalEffect/RandomForest_output_accuracyandError_1000trees_100permutations_redo.csv")
```
